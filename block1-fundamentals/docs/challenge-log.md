...
One file per module, and one for the capstone. **Numbering is continuous
across the whole block**, so that any challenge can be cited by number alone;
this table is how a bare number is resolved to a file.
...
warmed). The capstone adds a fourth instrument, the **bit pattern**:
`doubleToLongBits`, because the distinctions its Part VII turns on are ones
`==` cannot express.
