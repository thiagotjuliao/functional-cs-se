...
does not unlock the next module. Where a box asks for a measured number, the
number is written into that module's file — an unrecorded measurement is an
...
in [`error-patterns.md`](error-patterns.md) — the latter is organised by
pattern rather than by module, deliberately, because a pattern with
occurrences in two modules is a finding and splitting it would hide that.
